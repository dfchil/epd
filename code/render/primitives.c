...
void render_strip_line_col(shz_vec2_t *points, int point_count, shz_vec3_t *offset,
  ...
  shz_vec2_t bisector = shz_vec2_scale(shz_vec2_normalize(shz_vec2_add(v0, v0)),
                                       line_width);
  ...
  shz_vec3_deref(&(vert->x)) = render_perspective_div(shz_xmtrx_transform_vec4(
      shz_vec4_init(points[0].x - bisector.y, points[0].y + bisector.x,
  ...
  for (int i = 0; i < point_count - 2; i++) {
    ...
    bisector = shz_vec2_scale(shz_vec2_normalize(shz_vec2_add(v0, v1)), line_width);
  ...
  bisector = shz_vec2_scale(shz_vec2_normalize(shz_vec2_add(v0, v0)), line_width);
  vert = (pvr_vertex_t *)pvr_dr_target(state);
  shz_vec3_deref(&(vert->x)) =
      render_perspective_div(shz_xmtrx_transform_vec4(shz_vec4_init(
          points[point_count - 1].x - bisector.y,
          points[point_count - 1].y + bisector.x,
