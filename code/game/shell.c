...
shell_t *shell_create(float pos_x, float pos_y, float vel_x, float vel_y,
  ...
  new_shell->shadow_min_x = UINT32_MAX;
  new_shell->shadow_max_x = 0;
...
int shell_update(shell_t *shell, float delta_time) {
  ...
  const scene_t *scene = (scene_t *)shell->origin->scene;
  const terrain_t *terrain = scene->terrain;

  shell->shadow_min_x = UINT32_MAX;
  shell->shadow_max_x = 0;
  ...
  for (int i = 0; i < SHELL_SIMULATION_STEPS; i++) {
    ...
    for (int p = 0; p < scene->num_players; p++) {
      ...
      float collision_t =
          collision_player_line(player, &shell->position, &delta);
      ...
      if (collision_t >= 0.0f) {
        ...
        printf("Player %d hit by shell from player %p!\n", p,
               (void *)shell->origin->controller.port);
        confetti_create(shell->position, player->color.primary,
                        shell->origin->color.primary, 55, 2.0f, 90);

        if (player == shell->origin) {
          // Self-hit, sarcastic sound
          sfx_play(SFX_SARCASTIC, 128, sfx_pos2pan(shell->position.x));
        } else {
          // Hit another player, enthusiastic sound
          sfx_play(SFX_YAY_ENTHUSIASTIC, 128, sfx_pos2pan(shell->position.x));
        }
        shell->velocity = (shz_vec2_t){.x = 0.0f, .y = 0.0f};
    ...
    terrain_index_0 =
        (int)(SHZ_MIN(shell->position.x, shell->position.x + delta.x) /
              (vid_mode->width / terrain->num_verts));
    terrain_index_1 =
        (int)(SHZ_MAX(shell->position.x, shell->position.x + delta.x) /
              (vid_mode->width / terrain->num_verts));
    terrain_index_0 -= 1;
    ...
    shell->shadow_max_x = terrain_index_1 > shell->shadow_max_x
                              ? terrain_index_1
                              : shell->shadow_max_x;
    shell->shadow_min_x = terrain_index_0 < shell->shadow_min_x
                              ? terrain_index_0
                              : shell->shadow_min_x;

    for (int t = terrain_index_0; t <= terrain_index_1; t++) {
      float collision_t = collision_line_line(
          &shell->position, &delta, terrain->verts + t,
      ...
      if (collision_t >= 0.0f) {
        ...
        shell->trail[current_frame] = shell->position;
        shell->trail_fade = SHELL_MOTION_BLUR_STEPS;
        sfx_play(SFX_THUD, 128, sfx_pos2pan(shell->position.x));
        shell->moving = 0;
        render_shell(shell);
        shell->velocity = (shz_vec2_t){.x = 0.0f, .y = 0.0f};
        return 1; // Shell is still active
