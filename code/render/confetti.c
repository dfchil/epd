...
void _render_confett_TR(void *data) {
  ...
  for (int i = 0; i < cluster->num_particles; i++) {
    float angle = lifetime * 6.28318f * cluster->particles[i].rotation_speed + cluster->particles[i].start_angle;
    ...
    *(shz_vec3_t *)(corners[3]) = shz_quat_transform_vec3(
    ...
    shz_vec3_t v0 = shz_vec3_sub(
        (shz_vec3_t){corners[1][0], corners[1][1], corners[1][2]},
        (shz_vec3_t){corners[0][0], corners[0][1], corners[0][2]});
    shz_vec3_t v1 = shz_vec3_sub(
        (shz_vec3_t){corners[2][0], corners[2][1], corners[2][2]},
        (shz_vec3_t){corners[0][0], corners[0][1], corners[0][2]});
  
    hdr_s.argb = shz_vec3_cross(v0, v1).z < 0.0f
                      ? cluster->color_dst.raw
